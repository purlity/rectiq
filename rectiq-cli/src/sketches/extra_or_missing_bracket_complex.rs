// rectiq-cli/src/sketches/extra_or_missing_bracket_complex.rs
...
pub struct ExtraOrMissingBracketComplexSketcher;

impl Default for ExtraOrMissingBracketComplexSketcher {
    fn default() -> Self {
        Self::new()
    }
}

impl ExtraOrMissingBracketComplexSketcher {
    #[must_use]
...
impl TokenSketcher for ExtraOrMissingBracketComplexSketcher {
    ...
    fn finalize<'a>(&'a mut self, pool: &'a LocalShapePool) -> Option<SketchNode<'a>> {
        ...
        if spans.is_empty() {
            ...
            Some(SketchNode {
                kind: Kind::ExtraOrMissingBracketComplex,
                fix_hint: None,
